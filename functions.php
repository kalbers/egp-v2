...
/* http://calisza.wordpress.com/2009/09/24/howto-create-custom-thumbnails-from-MF-image-fields/ */


/* Enque scripts */

function egp_scripts () {
	wp_enqueue_script('jquery.tools.min', '//cdn.jquerytools.org/1.2.7/full/jquery.tools.min.js', array());
	wp_enqueue_script('jquery.color', get_template_directory_uri() . '/js/jquery.color.js', array('jquery'));
	wp_enqueue_script('egp', get_template_directory_uri() . '/js/egp.js', array());
}
add_action ( 'wp_enqueue_script', 'egp_scripts' );
...
function term_order_sort($a,$b) {
     return $a->term_order>$b->term_order;
}  
  
function cat_col_list($parent_id, $col_count, $exclude=NULL) {
  $essay_cat_id = $parent_id;
  $cats = get_categories('title_li=&echo=0&style=0&exclude='.$exclude.'&child_of='.$essay_cat_id.'&orderby=term_order');
  $cats_keys = array_keys($cats);

  $cnt_index = 0; 

  
  echo '<ul class="cat-col-list">';
  foreach ($cats as $cat) {
  //echo '<!--'.$cat->name.'-->';
    if (cat_is_ancestor_of($exclude, $cat->cat_ID) == true) {
      continue;
    }

      if ($cat->parent == $essay_cat_id) {
            $child_arr = array();
              //start parent item
    		  //this is a cheap way to show parent links
    		  if ($exclude) {
    			  echo '<li class="parent-item"><a href="'.get_category_link($cat->cat_ID).'">'.$cat->name.'</a>';
    		  } else {
         	     echo '<li class="parent-item">'.$cat->name;
    		  }
          //start child list
          echo '<div class="child">';
      } else {
        $child_arr[] = $cat;
        usort($child_arr, "term_order_sort");
        
       //if next is a parent, or the list is over
        if ($cats[$cats_keys[$cnt_index+1]]->parent == $essay_cat_id || $cnt_index+1 == count($cats) ) {

            $columns = $col_count; 
            $arraySize = count($child_arr); 
            $remainder=$arraySize%$columns;
            $colSize = $arraySize/$columns; 
            if ($remainder > 0) {
              $colSize = ceil($colSize);
            }
			
              for($i=1;$i<=$columns;++$i){
				 // THIS WAS A BUG, COMMENTING OUT.
				 //if this is the last column 
                //if ($remainder > 0 && $i == $columns) {
					// the remainder is not necesarily equal to the length of the column
					//11%3 = 2 remainder
					//columns should be 4+4+3 = 11 ... last columng is 3 not two.
                 	// $colSize = $remainder;
                //}
                ...
                for($j=0; $j<$colSize; ++$j ) {
	            	if ($child_arr[$j]->name) {
	                  echo '<li>';
	                    echo '<a href="'.get_category_link($child_arr[$j]->cat_ID).'">'.$child_arr[$j]->name.'</a>';
	                  echo '</li>';
	      		 	}
                }
                echo '</ul>';   
...
//echo '<!--'.$remainder.'-->';
                $child_arr = array_slice($child_arr, $colSize, $arraySize); 
              }
...
//REDUCE CATEGORY URLS TO MOST CHILD CATEGORY
